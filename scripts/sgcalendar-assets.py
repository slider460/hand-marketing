...
Источник: презентация ~/Downloads/SGabainCalendar_pres 2.pdf, 7 полос.
Картинки внутри PDF лежат в CMYK, поэтому берём их не рендером страницы,
а напрямую из потока (extract_image) и конвертируем в RGB: рендер страницы
пережал бы мокапы до 1920 px, а в потоке они по 5-6 тысяч пикселей.
...
CROPS = [
    ...
    ('calendar', 'mock-cover.jpg',  (0.783, 0.040, 1.000, 0.938), 900),
    ...
    # полоса 5: подарки
    ('gifts', 'card.jpg', (0.630, 0.000, 0.872, 0.845), 900),
]

# ─── листы-раскраски: (ключ, файл, кроп) ─────────────────────────────────────
# обложка календаря в презентации обрезана правым краем полосы, поэтому от неё
# берём квадрат из середины: получается бесшовное поле узора без рваной границы
SHEETS = [
    ('gifts',    'sheet-card.png',  (0.665, 0.372, 0.830, 0.706)),
    ('calendar', 'sheet-cover.png', (0.835, 0.331, 0.9915, 0.6103)),
]
...
if __name__ == '__main__':
    ...
    for key, name, box in SHEETS:
        save(lineart(cut(src[key], box)), name, 760)
