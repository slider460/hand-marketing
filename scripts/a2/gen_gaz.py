...
STORY = [
    ...
    ('Нога робота входит в грязь', 46.9),
    ...
    ('Маршрутка падает перед первой машиной', 66.2),
...
DIFFS = [
    ('Кнопка с листа', 'Кнопка с площадки',
     'На листах сториборда кнопка красная. Серийной кнопки блокировки на '
     'момент съёмки ещё не существовало: машину снимали раньше, чем опция '
     'встала на конвейер. Ту кнопку, что видна в кадре, собрали руками прямо '
     'на площадке.'),
    ...
    ('Одна смена', 'Четыре смены',
     'План был снять всё за одну смену без переработок. В итоге вышло четыре '
     'дня на двух локациях, причём последний выезд состоялся ради одного '
     'кадра: маршрутка падает сверху перед первой машиной на светофоре.'),
]
...
SHIFTS = [
    ('День 1 · бездорожье',
     'Съезд машины с трассы на разбитую колею и женщина на остановке'),
    ('Дни 2 и 3 · вторая площадка',
     'Пробка и все крупные планы: водитель, пассажиры, панель'),
    ('Доп. день',
     'Один кадр: маршрутка падает сверху перед первой машиной на светофоре'),
]
...
.gz-st.is-film .gz-st__a{opacity:0}
.gz-st.is-film .gz-st__b{opacity:1}
...
<h2 class="gz-h2">Из трёх направлений дописали одно</h2>
<p class="gz-sub">До съёмки сюжет искали в трёх направлениях. Два первых так и
остались набросками: их записали в несколько строк и дальше плана не повели.
Развернули в полный сценарий только третье.</p>
...
пэкшот, ниже они идут по порядку. Сняты все {len(STORY)}: каждый лист стоит
в ролике ровно так, как был нарисован.</p>
...
<h2 class="gz-h2">Двенадцать этапов и четыре смены</h2>
...
спорить уже не о чем. Выездов в итоге было четыре, на двух разных площадках.</p>
...
разбитой дорогой и с разрешённым съездом на неё, чтобы знак стоял в кадре и к
съёмке не было вопросов. Такую нашли недалеко от Москвы: здесь сняли первый
день, съезд с асфальта и женщину на остановке. Пробку и все крупные планы
снимали уже на второй площадке, тихой и непроездной, чтобы никто не мешал
работать с актёрами.</p>
...
<figure>{pic('f-20.jpg', 'Кадр ролика: нога робота входит в грязь, брызги во все стороны')}
<figcaption>Лучший кадр по графике, 0:46</figcaption></figure>
</div>
<p class="gz-sub" style="margin-top:18px">Тяжелее всего далась не сама модель,
а её контакт с землёй: нога робота входит в жидкую грязь, и брызги должны
лететь так же, как от настоящего удара по луже. Этот кадр в итоге и получился
лучшим в ролике по графике.</p>
</div></section>'''
