...
MOCKUPS = [
 ('mock-cover.jpg', 'Обложка и разворот про инвестора: квадрат 210×210 мм'),
...
def craft():
    return (
      ...
      f'<img src="{IMG}/spread-05.jpg" width="2500" height="1250" '
      'alt="Разворот брошюры Vertical: фотография яхты и пляжа, разрезанная диагональю через сгиб" '
      'loading="lazy">'
      '<figcaption>Диагональ идёт через оба разворота, поэтому сгиб перестаёт быть '
      'границей: две полосы читаются как один кадр.</figcaption>'
      '</figure></div>'
      '</div></section>')
