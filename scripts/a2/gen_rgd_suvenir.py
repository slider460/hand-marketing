...
целиком: семь позиций по договору, от бумажного пакета до ёлочного шара.
...
# ─── состав набора: семь позиций по договору ───────────────────────────────
# (слаг кадра или None, если предмет в съёмку не попал; имя; строка про то,
#  что с этим предметом делала айдентика; техника из спецификации коротко)
KIT = [
    ('bag', 'Бумажный пакет',
    ...
    ('diary', 'Ежедневник',
     ...
     'бумага от 120 г/м²'),
    (None, 'Плащ-дождевик',
     'Материал прозрачный, поэтому от айдентики остаётся только знак: '
     'разгон на бесцветной плёнке не читается',
     'EVA, прозрачный без цвета, нанесение знака по брендбуку ОАО «РЖД»'),
    ('mugs', 'Термокружка',
...
# (слаг кадра, имя, ключ силуэта) — дождевик сюда не идёт, на прозрачной
# плёнке разгона нет, а календарь показан отдельным блоком
CUTS = [
    ('bag', 'Пакет', 'bag'),
    ('box', 'Коробка', 'box'),
...
.rs-kit__none{width:100%;aspect-ratio:4/3;display:flex;align-items:center;
 justify-content:center;background:var(--paper);border:1px dashed var(--line);
 border-radius:2px}
.rs-kit__none span{font-size:13px;letter-spacing:.12em;text-transform:uppercase;
 color:var(--mute);opacity:.6}
...
def hero():
    facts = [
        ('7 позиций', 'Пакет, календарь, ежедневник, дождевик, кружка, '
                      'зарядное, шар'),
    ...
    return (
      ...
      'терминально-складским комплексом: семь позиций от бумажного пакета '
...
def brief():
    return (
      ...
      '<h3>Концепция, а не логотип на семи предметах</h3>'
      '<p>Собрать новогодний подарок дирекции: семь позиций набора, каждая '
...
def kit():
    """Состав набора: семь позиций из спецификации договора."""
    cards = ''
    for slug, name, what, spec in KIT:
        pic_html = (zoom(slug, '(max-width:760px) 100vw, '
                               '(max-width:1100px) 46vw, 360px')
                    if slug else
                    '<div class="rs-kit__none" aria-hidden="true">'
                    '<span>кадра нет</span></div>')
        cards += ('<div>{p}<h3>{n}</h3><p>{w}</p>'
                  '<p class="rs-kit__spec">{s}</p></div>').format(
                      p=pic_html, n=name, w=what, s=spec)
    return (
      '<section class="day"><div class="in">'
      '<div class="lbl">Состав</div>'
      '<h2>Семь позиций набора</h2>'
      '<p class="lead">Договор описывал набор списком: пакет, календарь, '
      'ежедневник, дождевик, термокружка, зарядное устройство и ёлочный шар. '
      ...
      '<p class="note" style="margin-top:26px">Плащ-дождевик в съёмку '
      'не попал, поэтому карточка идёт без кадра: показывать чужой '
      'предмет вместо своего мы не будем.</p>'
