...
CASES=[
 ("/portfolio/samara-stand-vdnh","Стенд Самарской области","ВДНХ, 16 млн посетителей стенда","/images/lib/custom-samara-vdnh/cover-main.png"),
 ("/portfolio/stavropol-stand-vdnh","Стенд Ставропольского края","Мультимедиа стенда на ВДНХ, приз зрительских симпатий","/images/lib/custom-stavropol-vdnh/cover-main.png"),
 ("/portfolio/samara-exhibition","Выставка «Самара»","Музей им. Алабина","/images/lib/custom-samara-exhibition/cover-main.png"),
 ("/portfolio/becar-private-money","Стенд You&Co для Becar","Private Money Expo Forum 2021, семь брендов Becar","/images/lib/custom-becar-pm/cover-main.png"),
 ("/eaton_online","Эфир Eaton на форуме «IT-ОСЬ 2020»","Семь часов трансляции в виртуальный стенд","/images/lib/as6438-6362-4632-b262-313335333833/image_2021-03-06_22-.png"),
]
STEPS=[
 ...
 ("Производство","Печать, дерево, металл, пластик, акрил, сборка каркаса"),
...
DRAFT_SVG="""<svg class="ex-draft" viewBox="0 0 560 420" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
<defs>
...
<text class="ex-note" x="120" y="398">От эскиза до монтажа</text>
</svg>"""
...
CONCEPTS = [
    dict(tag='Концепция А', name='Пять духов', video='/media/samara-pres-5spirits.mp4',
    ...
    dict(tag='Концепция Б', name='Четыре стихии', video='/media/samara-pres-4elements.mp4',
         ...
         essence='Четыре силы региона: земля, вода, огонь и воздух. У каждой свой цвет, свой звук и своя зона на стенде.',
...
ZONES = [
    ...
    ('zone-business', 'Деловая зона',
     'Сцена с большим LED-экраном, звуком и подиумом. Здесь проходят церемонии федерального уровня, презентации '
     'и подписание соглашений. Гостей развлекает робот-андроид, он делает гимнастические упражнения и устраивает шоу.'),
    ('zone-vip', 'VIP-переговорная',
     'Закрытая комната на 8 человек в тихой части стенда: большой экран, флаги, система для презентаций. '
     'Здесь делегации ведут переговоры и подписывают соглашения без публики.'),
    ('zone-kinetic', 'Кинетическое шоу из хоккейных шайб',
     'Шайбы в честь юбилея ХК «Лада» висят над зоной, у каждой своя лебёдка. В фоновом режиме под потолком плывут волны '
     'из шайб. В режиме шоу шайбы опускаются и складываются в объёмные фигуры, а в финале собираются в гигантскую «50». '
     'Звёзды могут расписаться на шайбах прямо во время шоу.'),
    ('zone-digital', 'Информационные зоны «Цифровая Самара»',
     'На вертикальных тач-пилонах собран общий портал: спортивный навигатор по области, инфраструктура, '
     'главные проекты и истории успеха. Рядом стоят «Голоса легенд», 3D-фигуры знаменитых спортсменов с видеоприветствиями.'),
    ('zone-alley', 'Зона спортивных клубов «Аллея Чемпионов»',
     'Девять аттракционов, где гость своими движениями управляет графикой на экранах. '
     'У каждого спортивного клуба региона свой аттракцион.'),
    ('zone-catch', '«Волжский улов»',
     ...
     'Пилон работает в двух режимах: имиджевый контент, который привлекает гостей, и сама игра.'),
    ('zone-tyaga', '«Самарский кач» с ожившими «Бурлаками на Волге»',
     'Силовой интерактив по картине Репина: участник тянет якорь на тросе, и баржа на ожившем полотне '
    ...
    ('zone-record', '«Самарский рекорд»',
     'Прыжок в длину с места. Разметка нанесена прямо на пол стенда, а экран сразу показывает результат '
     'и сравнивает его с забавными местными ориентирами.'),
    ('zone-gto', 'Зона «Сдай ГТО с чемпионом»',
     ...
     'показывает упражнения как личный тренер. За сданный норматив участник получает брендированный диплом со своим именем.'),
    ('zone-hero-path', 'Полоса препятствий «Путь Героя»',
     ...
     'меняются под вид спорта, например хоккей или баскетбол.'),
    ('zone-regata', 'Акватория «Самарской регаты»',
    ...
    ('zone-makers', 'Зона производителей',
     'Экспозиция «Сделано в Самарской области»: местные компании показывают новинки спортивного инвентаря, '
     'экипировки и технологий. Среди них «умная одежда» MioBody и решения центра НТИ.'),
]
...
def _concepts():
    ...
    for c in CONCEPTS:
        ...
        h += (f'<div class="sam-con ex-rev">'
              ...
              f'<img src="{c["poster"]}" alt="{c["name"]} (креативная концепция стенда Самарской области)" loading="lazy" width="960" height="540">'
    ...
    for i, (kind, src, poster, cap) in enumerate(frames, 1):
        ...
        if kind == 'video':
            h += (f'<figure class="sam-frame"><button type="button" class="vp-facade" data-video="{src}" data-title="{full}" '
                  ...
                  f'<img src="{poster}" alt="{full} (дизайн-проект стенда)" loading="lazy" width="800" height="450">'
            ...
            h += (f'<figure class="sam-frame"><button type="button" class="sam-zoom" data-img="{p}" data-title="{full}" '
                  ...
                  f'<img src="{p}" alt="{full} (дизайн-проект стенда)" loading="lazy" width="800" height="450"></button>'
                  f'<figcaption class="sam-frame__cap"><span>{cap}</span><span class="sam-frame__idx">{idx}</span></figcaption></figure>')
    return h + '</div>'
...
def _zones():
    ...
    for i, (img, name, desc) in enumerate(ZONES, 1):
        ...
        rows += (f'<div class="sam-zone ex-rev">'
                 ...
                 f'<img src="{p}" alt="{name} (рендер стенда Самарской области)" loading="lazy" width="800" height="450"></button>'
...
def case_narr():
    ...
    brief = ('<p class="sam-brief ex-rev">Сделать стенд региона <em>главным</em> на международном спортивном форуме '
             'на «Солидарность Самара Арене»: удивить VIP-гостей и собрать очереди посетителей.</p>'
    ...
    plan = ('<figure class="sam-plan ex-rev"><button type="button" class="sam-zoom" data-img="/images/exhibition/samara/plan-v1.jpg" '
            'data-title="План застройки «Неоновой арены», 204 м²" aria-label="Открыть план застройки">'
            '<img src="/images/exhibition/samara/plan-v1.jpg" alt="План застройки «Неоновой арены», 204 м²" loading="lazy" width="800" height="450">'
            '</button><figcaption class="sam-frame__cap"><span>План застройки «Неоновой арены», 204 м²</span></figcaption></figure>')
    ...
    content = ('<div class="sam-cnt"><figure class="sam-cnt__v ex-rev">'
               ...
               '<div class="sam-cnt__t ex-rev"><p>Вместе с проектом заказчик получил сценарий для каждого экрана: '
               'ролики с AI-образами великих самарских спортсменов, инфографику «Самара в цифрах», спортивный квиз для тач-панелей '
               'и правила, по которым графика переносится на любую поверхность стенда, включая медиапотолок и пол.</p>'
               '<p class="sam-dim">Как мы делаем такой контент, рассказываем на страницах <a href="/content/">Content</a> и <a href="/videoproduction/">Video Production</a>.</p></div></div>')
    ...
    return (f'<div class="sam-dark" id="ex-case"><div class="sam-in">'
            ...
            f'<h2 class="sam-h ex-rev">Проект стенда Самарской области:<br>от легенды до рабочих чертежей</h2>'
            f'<p class="sam-intro ex-rev">Разбираем по шагам концепцию стенда Самарской области к международному форуму '
            f'«Россия — спортивная держава». Стенд по ней не строили, но на проекте хорошо видно, '
            f'как мы работаем до начала производства.</p>'
            f'{hud}'
            f'{_mark("SMR·01", "Задача")}{brief}'
            f'{_mark("SMR·02", "Две концепции на выбор заказчика")}'
            f'<p class="sam-lead ex-rev">Мы разработали две концепции. У каждой своя легенда, свой визуальный код '
            f'и своя видеопрезентация. Нажмите на кадр, ролик откроется со звуком:</p>'
            f'{_concepts()}'
            f'{_mark("SMR·03", "Зоны стенда · 204 м²")}'
            f'<p class="sam-lead ex-rev">Проектируя стенд, мы разводим потоки. У VIP-делегации, деловых гостей и семей '
            f'с детьми свои маршруты и свои «сувениры на память». Экраны, интерактивные инсталляции и LED-пилоны '
            f'мы заложили в конструктив с первого чертежа. Пятнадцать зон этого проекта:</p>'
            f'{_zones()}{plan}'
...
FAQ = [
    ...
    ('Кто отвечает за изготовление выставочного стенда?',
     ...
     'а не собираются из типовых комплектов. Заказчик получает стенд для выставки, '
     'который совпадает с утверждённым 3D-проектом, и одного ответственного '
     'за результат на площадке.'),
    ('За сколько нужно заказать стенд для выставки?',
     'Комфортно за два-три месяца до выставки: концепция и дизайн-проект занимают '
     'от двух недель, изготовление и подготовка к монтажу ещё несколько недель. '
     'Быстрее тоже можно, если площадка и конструктив типовые.'),
...
# Квиз на странице НЕ выводится: 13.09.2026 владелец оставил только фиолетовую форму.
...
<div class="ex-quiz ex-rev" data-quiz>
 <p class="ex-quiz__s">Ответьте на четыре вопроса, это займёт минуту. Подготовим расчёт под вашу выставку и перезвоним с вариантами.</p>
...
<meta name="description" content="Застройка и изготовление выставочных стендов под ключ: дизайн и 3D-визуализация, своё производство, мультимедиа, монтаж в любом городе России. Стенды для выставки «Россия» на ВДНХ. От 500 000 ₽.">
...
def build():
    ...
    bundle=(
        ...
        '<a href="/exhibition/multimedia/">Мультимедиа для стенда</a>.</p></div>'
        '<div class="ex-step ex-rev"><span class="ex-step__n" aria-hidden="true"></span>'
