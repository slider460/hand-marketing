/* Адаптация зеро-блоков для планшетов/мобильных.
   В исходных макетах Tilda на разрешениях 480/640 часть текстовых элементов
   наложена друг на друга. Скрипт после рендера t396 находит пересекающиеся
   текстовые элементы и «вставляет» вертикальное пространство: всё ниже точки
   разрыва сдвигается, перекрывающие разрыв фоны растягиваются. */
/* Запрет автозапуска роликов.
   Снимает атрибут autoplay со всех <video>, ставит preload="none",
   и ставит на паузу любой ролик, запущенный не по клику пользователя
   (в т.ч. в попапах, которые Tilda добавляет в DOM динамически). */
(function () {
  'use strict'
