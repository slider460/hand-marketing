...
> **06.08.2026 — ролик кейса Ставрополья НЕ нужно грузить вручную.** `mirror/videos/stavropol-stand.mp4`
> (57 с, 8.4 МБ, адрес `/videos/stavropol-stand.mp4`) лежит в `mirror/**` и уезжает на хостинг обычным
> деплоем. Это короткая версия `media/stavropol-vdnh-main.mp4`, смонтированная без ведущего экскурсии
...
> **08.09.2026 — бекстейдж кейса «Газель-трансформер» НЕ нужно грузить вручную.**
> `mirror/videos/gaz-backstage.mp4` (45 с, 960x540, 4.8 МБ, адрес `/videos/gaz-backstage.mp4`)
> лежит в `mirror/**` и уезжает на хостинг обычным деплоем. Один непрерывный план с площадки:
> группа готовит кадр с актрисой на остановке (лист сториборда 19, в ролике 0:44). Режет и
> нормализует звук `scripts/gaz-assets.py --backstage` из папки «Материалы для обновления
> сайта/Eaton/Газель трансформер» (путь переопределяется через `GAZ_BACKSTAGE`).
> Сам ролик `/media/gazelle-transformer.mp4` на сервере уже лежит и не менялся.
...
> `samara-vdnh-kinetic-sport.mp4` (7.1 МБ), `samara-vdnh-kinetic-before.mp4` (7.0 МБ).
> Готовит их `scripts/samara-vdnh-assets.py` из папки «Эмуляции выборочных проектов
